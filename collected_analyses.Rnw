...
#log data
pcaDatl.mi <- apply(data.matrix(raw.w.mi[-c(1:5), which(names(raw.w.mi) %in% annodf.mi$SampleID)]), c(1,2), function(x) log2(x + 1))
pcplotdatl.mi <- plotPCA(pcaDatl.mi, annodf.mi, scale = FALSE)
ggpcal.mi <- ggplot(pcplotdatl.mi[[1]], aes(x= PC1, y= PC2, color=Plate, shape=SampType)) + 
    geom_point( size = 3)  + 
  annotate("text", label = paste0("Cum. proportion of var. = ", round(pcplotdatl.mi[[2]], 2)), x = 0, y = 0, color = "black") + 
...
ggpcac <- ggplot(pcplotdatc.mi[[1]], aes(x= PC1, y= PC2, color=Plate, shape=SampType, label = SampleID)) + 
    geom_point( size = 3)   + 
  # geom_text( size = 10) +
  annotate("text", label = paste0("Cum. proportion of var. = ", round(pcplotdatc.mi[[2]], 2)), x = 0, y = 0, color = "black") + 
  coord_equal()
