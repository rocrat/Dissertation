...
Once sequences have measured by the seqeuncer they are aligned to a genome and various quality metrics are calculated.  Alignment typically proceeds through a greedy algorithm implemented in programs such as Bowtie~\cite{}. Several different seqiences might aling with varying accruacy to the same gene and these must be counted in a way which accounts for the uncertainty of the alignment. In targeted sequencing the alignment is greatly simplified because the targets are known and consist of a much smaller set of possible sequences.\\   
