...
pcaDat.mi <- data.matrix(raw.w.mi[-c(1:5), which(names(raw.w.mi) %in% annodf.mi$SampleID)])
#remove 0 count probes
pcaDat.mi <- pcaDat.mi[-which(rowSums(pcaDat.mi) == 0), ]
pcplotdat.mi <- plotPCA(pcaDat.mi, annodf.mi, scale = TRUE)
ggpca.mi <- ggplot(pcplotdat.mi, aes(x= PC1, y= PC2, color=SampType, shape=Plate, label = SampleID)) + 
    geom_point( size = 3)  + 
  # geom_text(size = 2) + 
  coord_equal()
ggsave("./Paper1/Figures/miRNA_Repro_PCA_Raw(scaled).png", ggpca.mi, width = 9, height = 8 )
...
ggpcan.mi <- ggplot(pcplotdatn.mi, aes(x= PC1, y= PC2, color=SampType, shape=Plate, label = SampleID)) + 
    geom_point( size = 3) + 
  # geom_text(size = 1.7)  + 
  coord_equal()
...
pcaDatc.mi <- as.data.frame(t(clr(acomp(t(pcaDat.mi), detectionlimit = 0))))
pcplotdatc.mi <- plotPCA(pcaDatc.mi, annodf.mi, scale = FALSE)
ggpcac <- ggplot(pcplotdatc.mi, aes(x= PC1, y= PC2, color=SampType, shape=Plate, label = SampleID)) + 
    geom_point( size = 3)   + 
  # geom_text( size = 10) +
  coord_equal()
