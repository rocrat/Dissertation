...
The measurement of RNA expression in biological samples is a fruitful area of study in many areas of biology including fields as diverse as cancer research and evolutionary biology. Many different methods exist to measure expressed RNA such as micro-array, RT-qPCR, and next generation sequencing (NGS).  NGS-based methods are increasing in use but the error associated with NGS-based measurements are not well understood.  All existing NGS-based measuremnt methods involve multiple steps which create the potential for multiple sources of error. It is unlcear how these errorspropogate through the process and affect the final measurement.\\

Future advancements in the use of measured RNA for understanding biological systems depends on reliable measurements.  The necessary first step towards reliable measurements is understanding what the measurement error is and what factors influence it. No current methods exist to directly compare the accuracy and precision of NGS based RNA measurement systems or to evaluate the souces of this error.
...
Relative sensitivity is a statistical framework originally developed to evaluate measurements made in analytical chemistry.  Relative sensitivity has several useful statistical properties which make it a good candidate for this purpose.  \\
...
We will publish an open-source software suite to implement our method with the open course statistical language R.

\section{Aim 1: Direct Comparisons}
