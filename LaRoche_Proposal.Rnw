...
The theory of relative sensitivity was initially presented in John Mandel's book \emph{The Statistical Analysis of Experimental Data} (1984) but was never widely adopted in the analysis of measurement systems outside of analytical chemistry (with few references even within analytical chemistry).  We attribute this to the relatively spotty uptake of statistical procedures in general and to the limited utility of a such a method when calibration curves are more easlily atainable.  However, we believe the theory of relative sensitivity provides a simple, yet powerful, statistical framework for the evaluation of complicated NGS-based measurement systems.  \\
...
For a given measurment systme, steep slopes ($f(X)$) correspond to greater sensitivity because a steep slope results in large differences in the measured value for small differences in the corresponding property being measured (fig.~\ref{sensslope}).  The error around a sensitivity curve for a measurement process also affects the utility of the measurement.  For example, if the slope is small but there is little error then the measurement can still discriminate between different states of the property whereas large error can ``swamp out" a steeper slope (fig.~\ref{senserr}).\\
...
In order to construct and evaluate a sensitivity curve as described above one must know the true value of the property being measured.  For many NGS-based measurement systems this property is either unknowable or is exceedingly difficult to know.  However, relative sensitivity can be used to compare two measurement systems without knowing the true state of the underlying property being measured.  A key insight of Mandel's was that two measurments of the same quantity must be related.  Mandel used this insight to construct the  relative sensitivity curve as a way to compare two measurement systems, resulting in measurments $Y_1$ and $Y_2$, without knowing the true value of the measured quantity.  The relative sensitivity ratio takes the form:

$$RS(Y_1/Y_2) = \frac{\left| dY_1/dY_2 \right|}{\sigma_{Y_1}/\sigma_{Y_2}}.$$%be consistent with model terminology above

In this formulation, $dY_1/dY_2$ represents the slope of the relationship between $Y_1$ and $Y_2$, i.e. the slope of the line in a plot of $Y_1$ versus $Y_2$, while $\sigma_{Y_1}$ and $\sigma_{Y_2}$ represent the standard deviations of the two measurement systems.  This ratio does not involve the true value of the property being evaluated while providing a simple metric of relative utility for two measurements.  For most measurment systems the sensitivity of the measurement is not linear (fig.~\ref{sensslope}) and, therefore, the slope of the relationship between measurements $Y_1$ and $Y_2$ will also vary (fig.~\ref{relY1Y2}).  The resulting changes in the value of the relative sensitivity ratio produce the relative sensitivity curve (fig.~\ref{relsens}).  When the relative sensitivity is less than 1 method 2 is better whereas when the relative sensitivity is greater than 1 method 1 is better.  From figure~\ref{relsens} we can see that method 2 outperforms method 1 until the true value of the measured quantity is about 3.25, at which point method 1 becomes preferable.\\
...
%incorporate a direct comparison with previous method of evaluating normalization methods. %graphical explanation of the different normalization procedures.  More detail on the Dillies aet al paper.  Provide examples about how previous method 
