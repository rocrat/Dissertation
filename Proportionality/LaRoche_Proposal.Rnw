...
The CPM transformation can be simplified to the log of the compositional closure operation by removing the multiplication by $10^6$ and the addition of constants to both the numerator and denominator of the ratio.  The resulting operation can then be achieved through the matrix operation:

\begin{equation}
CLO(\mathbf{X}) = log( C_N \mathbf{X}),
\label{CPMmat}
\end{equation}
...
The difference in denominators has several consequences for the analysis of the resulting transformed data.  Dividing by the geometric mean in the CLR transformation arises naturally as a consequence of the log-transformation and is equivalent to subtracting the mean log-transformed value from each log-transformed component.
