...
<<Standard_Dev_miRNA, echo = FALSE>>=
raw.l.mi <- melt(as.matrix(raw.mi))
...
clr.l.mi <- melt(as.matrix(pcaDatc.mi))
...
probeAvg.cpm <- summarize(group_by(cpm.l.mi, Probe, type), avg = mean(value))
...
cpm.mi <- merge(sd.mi.cpm, probeAvg.cpm, by = c("Probe", "type"))
...
ggplot(cpm.mi, aes(x = avg, y = SD)) + geom_point() + facet_wrap(~type, ncol = 1) + ylab("Standard Deviation of log2(CPM) Values") + xlab("Average Expression of Probe (log2(CPM))")
min(brain.sd$SD)
@
