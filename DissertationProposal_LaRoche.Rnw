...
\section{Hypothesis}
I hypothesize that the application of relative sensitivity (Mandel 1984) will provide a novel method for evaluating the error associated with next-generation sequencing technologies.\\
...
\noindent\textbf{Aim 4}\\
Create and R pacakge that provides functions to estimate calibration curves and compare technologies.  \\
