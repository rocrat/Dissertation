...
The geometric mean as the denominator in the CLR transformation arises naturally from the log transformation of X.  Dividing by the geometric mean is equivalent to subtracting the mean log-transformed value from each log-transformed component thereby centering the data (equation~\ref{meanLog}).
